...
export default async function TravelPostPage({ params }: Props) {
  ...
  return (
    <main className="pt-24 min-h-screen text-[#e0e0e0]">
      <div className="relative h-[50vh] w-full overflow-hidden bg-gradient-to-br from-[#0f0f0f] via-[#141e30] to-[color-mix(in_srgb,var(--accent)_12%,transparent)]">
        {post.photos[0] && (
        ...
        <div className="absolute bottom-8 left-8 md:left-20">
          <h1 className="text-5xl font-extrabold text-[var(--accent)]">{post.city}</h1>
          <p className="mt-1 text-lg text-[color-mix(in_srgb,var(--accent)_48%,#94a3b8)]">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 py-12">
        <Link href="/travels" className="mb-8 inline-block font-mono text-sm text-[#555] transition hover:text-[var(--accent)]">
          ← Back to Travels
        </Link>
        <p className="mb-6 text-xl italic text-[color-mix(in_srgb,var(--accent)_42%,#94a3b8)]">{post.summary}</p>
        ...
        {post.photos.length > 1 && (
          <>
            <h2 className="mb-6 text-2xl font-bold text-[var(--accent)]">Photos</h2>
        ...
        <div className="border-t border-[#222] pt-8">
          <Link href="/travels" className="font-mono text-sm text-[var(--accent)] hover:underline">
            ← All Travels
          </Link>
        </div>
      </div>
